...
CnfWriter::CnfWriter(const std::string &file_name, const size_t num_clauses, const size_t max_var)
    : m_file(file_name), m_num_clauses(num_clauses), m_max_var(max_var) {
    if (!m_file.is_open()) {
        throw failure("failed to open the output file");
    }
    m_file << "c This file was generated by the dp program" << std::endl;
    m_file << std::endl;
    m_file << "p cnf " << num_clauses << " " << max_var << std::endl;
}
